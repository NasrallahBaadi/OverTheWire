...
def getpass(response):
    ...
    pattern = r"The password for natas1 is (.*)"
    # Using regex to search for password and extract it.
    passwd = (re.search(pattern, content).group(1)).split(" ")[0]
    print(f"The password of natas1 is: {passwd}")

def main():
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description="Grabs the password of natas1 from an HTML comment.")
    parser.add_argument('-p', '--password', help='Password of natas0.', required=True, metavar='Password')
